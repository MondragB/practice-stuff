...
function GameStreams({match, location}){
    ...
    useEffect ( () => {
        const fetchData =  async () => {
            const results = await api.get(`https://api.twitch.tv/helix/streams?game_id=${location.state.gameID}`);
            ...
            <div className='row'>
                {streamData.map(stream => (
                    <div className='col-lg-3 col-md-6 col-sm-12 mt-5'>
                        <div className='card'>
                            <img className='card-img-top' src={stream.thumbnail_url} alt='stream thumbail'/>
